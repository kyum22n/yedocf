...
const PopupNotice = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        const popupItems = response.data.filter(
          item => !hiddenIds.includes(String(item.noticeId))
        );
  ...
  const handleClosePopup = (popupId) => {
    if (!checkedPopups[popupId]) {
      ...
      setPopupNotices(prev => prev.filter(notice => notice.noticeId !== popupId));
    }
  };
  ...
  return (
    <>
      {popupNotices.map((notice) => {
        const top = '50%';
        const left = notice.noticeType === 'EVENT' ? '20%' : '80%';
        ...
        return (
          ...
          >
            ...
            <div style={{ display: 'flex', alignItems: 'center', margin: '15px 0' }}>
              <input
                type="checkbox"
                id={`checkbox-${notice.noticeId}`}
                checked={checkedPopups[notice.noticeId] || false}
                onChange={handleCheckboxChange(notice.noticeId)}
                style={{ marginRight: '8px' }}
              />
              <label
                htmlFor={`checkbox-${notice.noticeId}`}
